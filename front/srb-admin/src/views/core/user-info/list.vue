<template>
  <div class="app-container">
    ...
    <el-table :data="list" border stripe>
      ...
      <el-table-column prop="createTime" label="注册时间" width="100"></el-table-column>
      <el-table-column label="绑定状态" width="90">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.bindStatus === 0" type="warning" size="mini">未绑定</el-tag>
          <el-tag v-else-if="scope.row.bindStatus === 1" type="success" size="mini">绑定</el-tag>
      ...
      <el-table-column label="操作" align="center" width="200">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="primary" @click="showLoginRecord(scope.row.id)">
            登录日志
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-pagination :current-page="page" :total="total" :page-size="limit" :page-sizes="[10,20]" style="padding: 30px 0;" layout="total,sizes,prev,pager,next,jumper" @size-change="changePageSize" @current-change="changeCurrentPage"/>
  </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      fetchData(){
        userInfoApi.getPageList(this.page,this.limit,this.searchObj).then(resp=>{
      ...
      showLoginRecord(id){
        ...
        userInfoApi.getUserLoginRecordTop50(id).then(resp=>{
